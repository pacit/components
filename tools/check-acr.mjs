...
const renderAcr = (input, aggregates, scanned, sr) => {
  ...
  const passText = at.recorded
    ? `**Recorded.** ${at.readers.join(' and ')} over ${at.views}; the logs stand under \`${at.logs}\` (${input.atLogs.length} file${input.atLogs.length === 1 ? '' : 's'}), and every one of the ${total} component cards points its \`${SCREEN_READER_ROW}\` row at them.`
    : `**Not recorded.** The gates end where axe ends — the DOM and the CSS. What a screen reader says on arriving at each view is a question they do not answer, and this report says so rather than guessing: ${sr.gaps.length} of the ${total} component cards carry a \`${SCREEN_READER_ROW}\` row that reads \`none — gap\`, ${sr.measured.length} carry a reading, and ${sr.missing.length} have no row yet. The pass this report waits for: ${at.readers.join(' and ')}, over ${at.views}, one log per reader under \`${at.logs}\`, the announcement transcribed per view. The day the claims say \`"recorded": true\`, point 7 of the gate demands the logs on disk and a reading in every card. Until then the site says "machine-audited" and never "conformant" (decision 0061).`;
