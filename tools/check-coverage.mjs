 ...
 *  3. COMPLETE: every source file the git index holds — templates included — is in the report,
...
/**
 * What counts as "library code": every `.ts` and `.html` the git index holds under the
 * library, less the categories in `NOT_A_SOURCE`. The inventory is deliberately NOT read
 * from `coverageInclude` in `project.json`: were it to come from the configuration,
 * narrowing that configuration would remove a file from both sides of the comparison at
 * once and point 3 would stop seeing anything. An independent definition makes a narrowed
 * `coverageInclude` show up as a file missing from the report — that is, as a failure.
 *
 * Nor is it a list of patterns of the gate's own. Until 2026-09-23 it was — four globs over
 * `src/` and a fifth for the migrations — and a line struck off that list was a narrowing
 * no prepared input could see: the cases hand the check their sources ready-made, while the
 * list globbed the real tree, so nothing prepared ever reached it. Taking the fifth line
 * away left 145 files instead of 146 and the gate green both ways — `lesson-45` by a
 * pattern's absence rather than a test's. The index has no line to strike: a file the run
 * must measure is any file that exists, and the only way out of the denominator is a
 * category below, with its reason beside it and a case under it that passes the moment
 * the category grows (`migration-outside-report.json`).
 */
const CODE = /\.(ts|html)$/;

/**
 * The exceptions — CATEGORIES rather than files, and every one with the reason it is one,
 * because a silent list of exceptions is exactly the defect this gate stands against. The
 * reference input carries one file of each, none of them in the report: that the gate stays
 * quiet on them is measured there, and a category struck off fires there on point 3.
 */
const NOT_A_SOURCE = [
  // Anything that is neither code nor a template: styles, manifests, policies, snapshots.
  (p) => !CODE.test(p),
  ...
  // The mutation run's own harness: it is what RUNS the specs under Stryker, in a Vitest
  // configuration of its own that collects no coverage, and no spec of the `test` target
  // imports it. `check-mutation` excuses it from the other measurement for the same reason.
  (p) => p === `${PROJECT}/mutation.setup.ts`,
  // The `ng add` schematic — and that alone, not the whole of `schematics/`. It runs once,
  // in the consumer's CLI at install time, and it is measured where it runs: `check-consumer`
  // installs the package into a real application and executes it there. An `ng update`
  // migration has no such second measurement, because nothing in this workspace executes
  // one, so it stands under THIS floor with the components, and `coverageInclude` names its
  // directory for that. The category is spelled `ng-add/` and not `schematics/` on purpose:
  // widened back, the migration leaves the denominator and `migration-outside-report.json`
  // passes — which is the whole of what that case measures.
...
const librarySources = (inRepo) =>
  (Array.isArray(inRepo) ? inRepo : [])
    .filter((p) => typeof p === 'string' && !NOT_A_SOURCE.some((no) => no(p)))
    .sort();
 ...
 *   `inRepo` — every file of the library as the git index lists it; the files that MUST be
 *   in the report are what `NOT_A_SOURCE` leaves of it,
 ...
 */
const checkCoverage = ({ report, inRepo, target, exceptions }) => {
  ...
  const sources = librarySources(inRepo);
  if (!sources.length)
    throw new CoverageError(
      'sources',
      `not a single source file under ${PROJECT} in the git index, once ` +
        `NOT_A_SOURCE has had its say — point 3 would then always pass, having ` +
        `nothing to look for in the report`,
    );
  ...
  if (missing.length)
    throw new CoverageError(
      ...
      `${missing.length} source files are missing from the coverage report — ` +
        ...
        `its component, and an import will not do it. A file outside every directory ` +
        `\`coverageInclude\` names is reported by nobody: name the directory there, or ` +
        `excuse the file's CATEGORY in NOT_A_SOURCE with a reason.`,
    );
...
/** The library's files off the git index — the denominator nobody can narrow by a pattern. */
const inIndex = (path) =>
