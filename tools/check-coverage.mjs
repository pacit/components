...
const SKIPPED = [
  ...
  // Pure types — they vanish in compilation, not one executable line. The name alone does not
  // make that so: `check-files` point 8 holds every `*.types.ts` of the library to it.
...
const NOT_A_SOURCE = [
  ...
  // The tests themselves.
  (p) => p.endsWith('.spec.ts'),
  // Types, as `SKIPPED` has them, and pure because `check-files` point 8 holds them so — the
  // premise this category, `SKIPPED` and `coverageExclude` all stand on.
  (p) => p.endsWith('.types.ts'),
