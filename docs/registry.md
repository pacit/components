...
| [`req-project-tree-shaking`](requirements/project.md#req-project-tree-shaking) | ✅ enforced | `tools/check-bundle.mjs` (target `check-bundle` in `components`, in C… | `tools/check-bundle.fixtures/` — 24 doctored inputs, each rejected on… |
...
| [`req-project-language`](requirements/project.md#req-project-language)         | ✅ enforced | `tools/check-language.mjs` (target `check-language` in the root proje… | `tools/check-language.fixtures/` — 30 doctored inputs, each rejected … |
