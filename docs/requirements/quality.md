...
**Control:** `tools/check-coverage.fixtures/` — twenty-two doctored inputs, one per way of
disarming the gate (no report, a report another checkout wrote, one a checkout nested in
this one wrote, and the three such a report is not: every file elsewhere but one, keys left
absolute here, no file at all; an empty source list with an empty listing, an empty listing
