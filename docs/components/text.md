...
| RTL                             | `tools/check-styles.mjs` (point 5, [`req-token-logical`](../requirements/tokens.md#req-token-logical)) over the stylesheet, plus the `text-types-rtl` baseline in `apps/sandbox-e2e/src/visual.spec.ts`                                                                                                                                                                                                                                                                                                                                                          |
...
| Screen-reader log               | Read 2026-09-16 from `docs/acr/at/` — three readers, whole sandbox, and **the input `type` reaches the announcement**: VoiceOver names it outright (`E-mail john@example.com email field`, `Search… search text field`, `Password secure text field`), NVDA gives the behaviour instead (`E-mail, edit, has auto complete, john at example dot com`, `Password, edit, protected`), Orca sits between (`Password · password text.`, `Search · entry · Search…`). A password value is spoken by none of the three, which is the one thing here that had to be true |
| docs page                       | `/components/text` on the published site — prerendered, the demo's own source is the code tab                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
