...
ring that gives the groove its extent under `forced-colors: active` is drawn on the host — the
...
| `forced-colors: active` — no state carried by colour alone | `libs/components/progress/src/progress.scss` — the fill and the band are `Highlight`, the groove `Field`, and its extent a `CanvasText` outline: an outline and not a border because the fill is positioned over the groove, and on the HOST and not on the `<progress>` because the host clips the band and a clip erases a descendant's outline (measured — the two boxes are one rectangle, so the ring lands where it always did). The reading in the sheet: `libs/components/progress/src/progress.spec.ts`. In a browser: `apps/sandbox-e2e/src/forced-colors.spec.ts` — **and that one is owed a re-point**: it reads `outline-color` on the `track` part, where the ring no longer is, and `outline-color` with no outline computes to `currentColor`, which in this mode is `CanvasText` — so the assertion now passes on the fallback and asks nothing. The mechanism this refuses is a gradient band, dropped outright in that mode |
