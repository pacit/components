...
channel — an empty badge is a dev-mode warning, and forced colours drop every tone to one
...
| **Inputs**  | `tone` (`PctTone \| null`, default `null`) — the library's four names and the absence of one; the absence is the neutral, and it carries no `data-pct-tone` at all. The type is imported, never redeclared (0076)    |
...
| **Tokens**  | the `--pct-badge-*` prefix — a base triple plus one per tone — and fourteen entries in `contrast.policy.json`; the skin gained `on-danger` for this component, and `on-success` / `on-warning` / `on-info` with 0082 |
| **Strings** | **none.** The fourth component in the library to add nothing to `PCT_TEXTS` — the word is the consumer's                                                                                                             |

**Why the tones are not this component's list.** For the first release they were: `tone` took
`PctBadgeTone`, `'neutral' | 'danger'`, because `danger` was the only tone the skin had a
colour for and inventing three ramps at a component's feet would have put the skin's centre of
gravity in the wrong file
([0019](../decisions/0019-primitives-are-not-the-contract.md)). That union named the missing
ramps as its condition for growing; they landed with the button's tone axis
([0082](../decisions/0082-a-tone-on-a-button-is-a-face-and-the-label-is-what-speaks.md)), and
the list was spent rather than widened — two vocabularies for one idea is what
[0076](../decisions/0076-a-tone-is-two-channels-and-four-names.md) exists to prevent.
`neutral` went with it: a badge with no tone takes no `tone`, and wears exactly what
`'neutral'` wore.
...
| `forced-colors: active` — no state carried by colour alone | `libs/components/badge/src/badge.scss` — every tone drops to `CanvasText` on `Canvas` with the border standing, which is the component's own argument made visible. One `:host` rule inside the media block is enough, and **the instrument for that is static**: a tone rule here assigns `--pct-badge-*` and paints no property of its own, so `check-styles` point 7 finds nothing of a higher specificity for the block to lose to. No e2e case could say this — Chromium and Firefox substitute colours in the mode whichever rule won, and webkit is excluded from the suite ([`lesson-70`](../lessons.md#lesson-70)). What `apps/sandbox-e2e/src/forced-colors.spec.ts` does read is the other half: every tone arrives at the palette's one word, and the edge is still drawn |
...
| Tokens registered + an entry in `contrast.policy.json`     | `libs/tokens/src/contrast.policy.json` — fourteen entries: every word over its box (AA error), every toned box and its edge against the page (UI error — the box IS the statement), the untoned edge (UI warn); `libs/tokens/tokens.snapshot.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| Strings through `PCT_TEXTS`                                | not applicable — the component draws no text of its own                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| Entrypoint size budget                                     | `libs/components/size.snapshot.md` — `./badge` reaches for no other entrypoint at all: `PctTone` comes in under `import type`, which the build erases, so the internal-deps column stays `-` and the row moved on its own bytes only. No `PCT_TEXTS` key either, so not one other row of the snapshot moved                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| Unit + mutation                                            | `libs/components/badge/src/badge.spec.ts` — 8 cases; `libs/components/mutation.snapshot.md` — `badge.ts` measured in the same full run as the avatar's row                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |

## Decisions this component implements

[0053](../decisions/0053-a-badge-is-a-word-wearing-a-tone.md) (the main one — which of the
two things called "badge" this is, why the tone never speaks alone, why the list started at
two and ended at none of its own),
[0076](../decisions/0076-a-tone-is-two-channels-and-four-names.md) (the four names, decided
once for every component that would ever want them),
[0082](../decisions/0082-a-tone-on-a-button-is-a-face-and-the-label-is-what-speaks.md) (the
ramps that let this component stop waiting),
