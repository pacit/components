...
| **Inputs**  | `pctHero` (`'edge' \| 'text' \| 'fill' \| null`, default `null`) — one face per element, so gradient text on a gradient fill is unrepresentable rather than discouraged; `show` (`'always' \| 'interact'`, default `'always'`)                                                          |
