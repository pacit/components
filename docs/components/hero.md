...
| Screen-reader log                                          | Read 2026-09-16 from `docs/acr/at/` — three readers, whole sandbox. The whole card arrives in one announcement, heading included: VoiceOver reads `link heading level 3 Under attention The rim is drawn and hidden, so the reveal is an opacity.` and Orca the same text as a visited link. NVDA adds where it goes — `Read the case, visited, same page, link`. So the hero is one link to a reader, with its heading and its body inside the name, which is what the pattern promises |
| A docs page with live examples                             | `apps/sandbox/src/app/views/hero/` (the sandbox view). The published site: `/components/hero` — prerendered, the demo's own source is the code tab                                                                                                                                                                                                                                                                                                                                       |
