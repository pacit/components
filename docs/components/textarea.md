...
| Screen-reader log               | Read 2026-09-16 from `docs/acr/at/` — three readers, whole sandbox. All three announce the label, the value and the hint; **only NVDA says it is multi-line**: `A plain textarea, for comparison, edit, multi line, This one keeps its two lines and scrolls., blank`. VoiceOver names the role `text entry area`, Orca says `entry`, and neither distinguishes it from a single-line field. Nothing in the logs separates the autosizing box from the plain one, which is correct — the height is not a promise to a reader |
