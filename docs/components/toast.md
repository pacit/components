...
| Screen-reader log               | Read 2026-09-16 from `docs/acr/at/` — three readers, whole sandbox, and **the walk does not ask this question**. It presses Tab and nothing else, so no toast in these logs was ever raised: all three read the triggers only (`Save the draft · button.`). Whether a `log` is read one message at a time, and whether an `alert` nested in it interrupts, needs a pass that ACTS. What the logs do show is that the viewport costs no tab stop when empty — no reader announced one |
