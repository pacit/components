...
| **Tokens**      | the `--pct-toast-*` prefix plus thirteen entries in `contrast.policy.json` (five untoned, two per tone). Two of them exist for no other component: `--pct-toast-inset` (the distance from the window's edge, one number for both axes) and `--pct-toast-z-index`, whose job is a fallback — see the limitations                                                                                                              |
...
| **Strings**     | two, both through `PCT_TEXTS`: the cross's accessible name (`toastDismiss`) and the name of the stack as a REGION (`toastRegion`, since 4.16 — a place the keyboard can be sent to needs something to announce on arrival), read at render time ([0014](../decisions/0014-texts-as-signal.md)). A second key rather than the dialog's `dialogClose`: "close" is what a panel does and "dismiss" is what happens to a message |
...
- **A tone is not announced.** `tone` reaches the eye and no further: the mark is drawn inside
  ...
  a decision no single component should make. Two components wanted the same four (plan 4.14)._
...
- **The stack is at the end of the page's tab order, and the key that jumps to it is the
  ...
  ([0072](../decisions/0072-a-region-key-is-the-consumers-to-install.md), plan 4.16). The
