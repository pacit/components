...
| Visual screenshot               | `apps/sandbox-e2e/src/visual.spec.ts` — `drawer-docked`, of the **viewport** and not of a card: what a drawer looks like is a relation between the panel and the window                                                                                                          |
...
- **`position: fixed` means the window — unless an ancestor says otherwise.** A `transform`, a
  `filter`, `contain: paint` or `container-type` on any element above the drawer makes that
  element the containing block, and the panel docks to **it** instead. This is the platform's
  rule and not this component's, and it is the price of being drawn where the consumer wrote it
