...
| Visual screenshot                     | `apps/sandbox-e2e/src/visual.spec.ts` — `drawer-docked`, of the **viewport** and not of a card: what a drawer looks like is a relation between the panel and the window — the sandbox's navigation blanked by the stage, so a row in that list is not a pixel here ([`req-quality-e2e`](../requirements/quality.md#req-quality-e2e))                                                                                                                                                                                |
...
| Screen-reader log                     | Read 2026-09-16 from `docs/acr/at/` — three readers, whole sandbox. **Half answered.** The trigger carries its state to all three while the drawer is shut: `Sections · collapsed button.` (Orca), `Sections, from further down, button, collapsed` (NVDA), `Sections collapsed button` (VoiceOver). Whether the named `region` is then offered in a reader landmark list while the trigger says `expanded` is NOT in these logs — the walk never opened one — and that is now the exact shape of what is left      |
| docs page                             | `/components/drawer` on the published site — prerendered, the demo's own source is the code tab                                                                                                                                                                                                                                                                                                                                                                                                                     |
