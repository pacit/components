...
| **Parts**       | `panel`                                                                                                                                                                                                                                                                                                                            |
| **Harness**     | `PctTooltipHarness`, `PctTooltipPanelHarness`                                                                                                                                                                                                                                                                                      |
...
| `prefers-reduced-motion`        | `apps/sandbox-e2e/src/tooltip.spec.ts` — the duration read from the panel is the reduced one, and the leave still ends. The first component here with a real enter/leave, which is what the dialog's card said would arrive with it                                                                                                                                                                                                                                                                                                                       |
...
| Screen-reader log               | Read 2026-09-16 from `docs/acr/at/` — three readers, whole sandbox, and **this one is answered**. A described control takes focus and all three say the description, after the name and the role: Orca `Publish · button. · Runs every check before publishing.`, NVDA `Publish, button, Runs every check before publishing`, VoiceOver `Publish Runs every check before publishing button`. The undescribed button beside it reads `Approve the release, button` and nothing more, so the description is attributable to the tooltip and not to the page |
| docs page                       | `/components/tooltip` on the published site — prerendered, the demo's own source is the code tab                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
