...
| the touch-target floor                       | `accordion-item.scss` | `min-block-size: var(--pct-accordion-heading-target-min)`  | **nothing in the accordion suite** — only the visual baseline, and for the wrong reason. The 24 px case stays green because the padding clears 24 px without help: it holds the promise, not this line (plan 4.44) |
