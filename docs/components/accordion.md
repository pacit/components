...
| axe audit                        | `apps/sandbox-e2e/src/a11y.spec.ts` — the `/accordion` view, LTR and RTL, with all five demos of the sandbox on the page at once (open, exclusive, disabled, an `h4` level and a lone section); and a stage of its own in the derived list of panels — a `<details>` is a `panel` part the sandbox holds open, so the stage holds the claim that one stands open in the walk rather than an audit of its own (plan 4.28)                                                                                                                                                   |
...
like a true statement about a component (plan 4.19). **Eight of the twelve lines below live in
...
| the touch-target floor                       | `accordion-item.scss` | `min-block-size: var(--pct-accordion-heading-target-min)`  | `target-min.spec.ts` — since 4.44 closed. Before it, **nothing**: the 24 px case stays green with this line deleted, because the padding clears 24 px without help. The new case zeroes the padding token and empties the row, so the floor is the only thing left; deleting the line reads 0 px |
