...
| **Harness**     | `PctAccordionHarness`, `PctAccordionItemHarness`                                                                                                                                                                                                                                                                              |
...
| axe audit                        | `apps/sandbox-e2e/src/a11y.spec.ts` — the `/accordion` view, LTR and RTL, with all five demos of the sandbox on the page at once (open, exclusive, disabled, an `h4` level and a lone section); and a stage of its own in the derived list of panels — a `<details>` is a `panel` part the sandbox holds open, so the stage holds the claim that one stands open in the walk rather than an audit of its own ([`lesson-196`](../lessons.md#lesson-196))                                                                                                                                                          |
...
| Touch target ≥ 24×24 px          | `apps/sandbox-e2e/src/accordion.spec.ts` — every heading row's box measured outright; the floor is `--pct-accordion-heading-target-min` on the row itself, so what a finger gets is what an eye sees. The disarming below narrowed what that case holds — with the floor taken out the row still clears 24 px on its padding alone, so it measures the PROMISE and not this line — and `apps/sandbox-e2e/src/target-min.spec.ts` is what measures the line: with the padding token at zero and the row emptied, the box reads 24 px, and with the floor deleted it reads 0                                       |
...
| Screen-reader log                | Read 2026-09-16 from `docs/acr/at/` — three readers, whole sandbox. The question was what a reader says about a `DisclosureTriangle` with a heading inside it, and **only one of the three says the heading at all**: NVDA reads `Payment, heading, level 3, button, collapsed`; Orca reads `Payment · collapsed button.` and names no heading; VoiceOver reads `Shipping expanded summary` — the element name, not a mapped role. State is announced by all three, and the disabled item is `collapsed button grayed` under Orca. VoiceOver leaves the view after one stop here, so its reading covers one item |
| docs page                        | `/components/accordion` on the published site — prerendered, the demo's own source is the code tab                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |

## Negative controls

The table above names, for each promise, the case that holds it. This is the other half, and
it is a **measurement rather than a reading of the code**: every mechanism this component
actually writes was taken out by hand, one at a time, and the suite was run over the hole.
What turned red is what that line is holding. Where nothing turned red, the promise above
stands on something else, and the row says so.

It exists because of what this component is. Two files, 74 lines of code between them, and
everything else is the platform ([0046](../decisions/0046-a-disclosure-is-the-platforms-and-so-is-the-group-it-belongs-to.md)):
the mutation run — the tool that usually answers "would anything notice?" — reads only those
74 lines, so a green score here is a true statement about a small thing that looks exactly
like a true statement about a component ([`lesson-174`](../lessons.md#lesson-174)). **Eight of the twelve lines below live in
a template or a stylesheet, where no mutant is ever thrown.**

Measured on 2026-09-07 in chromium, against `apps/sandbox-e2e/src/accordion.spec.ts` (12 cases
in this engine), the three cross-cutting cases that name this component (axe, forced colours,
the visual baseline) and the 1174-case unit suite.

| mechanism                                    | where                 | disarmed by                                                | what turned red                                                                                                                                                                                                                                                                                                               |
| -------------------------------------------- | --------------------- | ---------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| the group's name reaches the section         | `accordion-item.html` | `[attr.name]="groupName()"` removed                        | e2e _one section at a time is the browser closing the others_; unit _gives its sections one name, and the others none_                                                                                                                                                                                                        |
| the heading level comes from the group       | `accordion-item.ts`   | the `headingLevel` computed pinned to `3`                  | e2e _the heading level is the page's, and it is a real heading_; unit _follows the group, and a section outside one keeps the default_                                                                                                                                                                                        |
| the press can be refused                     | `accordion-item.ts`   | `preventDefault()` dropped from `onPress`                  | e2e _a disabled section refuses the press and keeps everything else_; unit _refuses the press and says so_                                                                                                                                                                                                                    |
| the refusal is announced                     | `accordion-item.html` | `[attr.aria-disabled]` removed                             | the same two cases — one case holds both halves of "refused", the behaviour and the word for it                                                                                                                                                                                                                               |
| a section opens because the consumer said so | `accordion-item.html` | `[open]="open()"` removed                                  | e2e _several sections stand open at once unless the group says otherwise_; axe _a section standing open in the walk_; visual _accordion-stack_; unit _opens a section the consumer asks for_                                                                                                                                  |
| the platform's toggle is written back        | `accordion-item.ts`   | `open.set(…)` dropped from `onToggle`                      | **the unit suite alone** — _reports the change back through the two-way binding_. No browser case: they press and then read the ELEMENT, which the platform updates whether we listen or not                                                                                                                                  |
| two accordions on a page are two groups      | `accordion.ts`        | `nextPctId('pct-accordion')` replaced by a constant        | **the unit suite alone** — _gives its sections one name, and the others none_. A second accordion on one page is a thing no sandbox view draws                                                                                                                                                                                |
| the touch-target floor                       | `accordion-item.scss` | `min-block-size: var(--pct-accordion-heading-target-min)`  | `target-min.spec.ts` — since the floors were measured one at a time. Before that, **nothing**: the 24 px case stays green with this line deleted, because the padding clears 24 px without help. The new case zeroes the padding token and empties the row, so the floor is the only thing left; deleting the line reads 0 px |
| the marker sits at the far end of the row    | `accordion-item.scss` | `flex: 1 1 auto` off the title                             | e2e _the marker changes sides under dir="rtl"_; visual _accordion-stack_                                                                                                                                                                                                                                                      |
| the engine's own triangle is taken away      | `accordion-item.scss` | `list-style: none` replaced                                | e2e _the engine's own marker is gone and ours is drawn_                                                                                                                                                                                                                                                                       |
| a refused section in forced colours          | `accordion-item.scss` | the `[data-pct-disabled] { color: GrayText }` rule removed | e2e _a section nobody may open says GrayText, and the rest say CanvasText_                                                                                                                                                                                                                                                    |
| the marker takes the row's colour there      | `accordion-item.scss` | `.pct-accordion__marker { color: inherit }` removed        | the same case — the half of it that exists because the engine once substituted the marker on its own                                                                                                                                                                                                                          |

**What this does not say.** It is one engine: the cases run in three and the disarming was
taken in chromium, so a mechanism held only by gecko or webkit would read the same here.
And nothing below disarms **the platform** — the press, `Enter` and `Space`, the tab stop,
`aria-expanded`, find-in-page in a closed section — because there is no line of ours to take
out. That is what the component is for, and it is the reason this table is twelve mechanisms
rather than a list of the component's promises.
