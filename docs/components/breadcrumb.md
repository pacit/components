...
| Screen-reader log                                          | Read 2026-09-16 from `docs/acr/at/` — three readers, whole sandbox, and **the current page is announced as current, by name**: `Data · link. · (Current page)` (Orca), `Data, link, current page` (NVDA). The landmark and the list are announced on entry — `navigation · Breadcrumb · List with 3 items` (Orca), `link Home list 3 items` (VoiceOver) — so the trail is a list inside a named navigation to a reader, which is the whole promise. VoiceOver leaves this view after one stop, so its reading covers the first link |
| A docs page with live examples                             | `apps/sandbox/src/app/views/breadcrumb/` (the sandbox view). The published site: `/components/breadcrumb` — prerendered, the demo's own source is the code tab                                                                                                                                                                                                                                                                                                                                                                      |
