...
| Screen-reader log                                          | Read 2026-09-16 from `docs/acr/at/` — three readers, whole sandbox. **The one tab stop is confirmed by the instrument itself**: all three walks of this view end after a single Tab, because the roving `tabindex` leaves nothing else to land on. Orca announces `README.md. · tree level 1` and VoiceOver `README.md outline row (1 of 5)` — name, role and, for VoiceOver, the position. The walk never presses an arrow, so the announcement on MOVING through the walk is not here |
