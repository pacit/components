...
| RTL                             | partly — `tools/check-styles.mjs` (point 5, [`req-token-logical`](../requirements/tokens.md#req-token-logical)) over the stylesheet — one justified `pct-exception`, the symmetric `left: 50%` of the hit zone — and the `radio-in-wrapper-rtl` baseline in `apps/sandbox-e2e/src/visual.spec.ts`. **Not yet:** with `orientation="horizontal"` the Left/Right arrows have to swap, and no test presses them under `dir="rtl"`                                                                                                                  |
...
| Screen-reader log               | Read 2026-09-16 from `docs/acr/at/` — three readers, whole sandbox. Orca and VoiceOver give the whole shape: `Plan · panel · Free · not selected radio button.` and `Option A selected radio button, 1 of 2 Horizontal layout radio group` — group, option, state, and for VoiceOver the position. **NVDA announces the group and stops**: `Horizontal layout, grouping`, with no option and no state, at every radio stop of this walk. Whether that is NVDA in browse mode or the group itself is the next question, and it is a specific one |
| docs page                       | `/components/radio` on the published site — prerendered, the demo's own source is the code tab                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
