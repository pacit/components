...
| Parts in the inventory          | `libs/components/parts.snapshot.md`, `tools/check-parts.mjs` (target `check-parts`). Note: `label` collided with the options' labels — hence the `group-` prefix ([`lesson-15`](../lessons.md#lesson-15))                               |
