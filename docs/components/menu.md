...
| **Harness**     | `PctMenuHarness`, `PctMenuItemHarness`, `PctMenuTriggerHarness`                                                                                                                                                                                                                                                                                                                                                                                |
...
| Visual screenshot               | `apps/sandbox-e2e/src/visual.spec.ts` — `menu-open`, with the submenu open: two panels of the page's own surface drawn side by side, where the edges and the shadows are all there is between them and the page — the sandbox's navigation blanked by the stage, so a row in that list is not a pixel here ([`req-quality-e2e`](../requirements/quality.md#req-quality-e2e))                                                                                                                                          |
...
| Screen-reader log               | Read 2026-09-16 from `docs/acr/at/` — three readers, whole sandbox. The walk presses Tab and never opens a menu, so **no row of one was reached and that half stands open**. What the logs do settle is the TRIGGER: all three name it and say that it opens a menu — Orca `Actions · collapsed button. · opens menu`, NVDA `File, button, collapsed, sub Menu`, VoiceOver `Actions menu pop up button` — and the nested triggers read the same way as the top one, which is what `aria-haspopup` on a submenu is for |
| docs page                       | `/components/menu` on the published site — prerendered, the demo's own source is the code tab                                                                                                                                                                                                                                                                                                                                                                                                                         |
