...
opened it, Escape from the closing stack. With `inline` it steps out of that pattern on purpose
and is a plain non-modal `role="dialog"` — see **Inline** below
...
The same component as a section of the page — the panel drawn where the tag stands, with no
layer over anything:

```html
<pct-dialog inline heading="Filters" [open]="true">
  <p>The page around this goes on answering, and so does the keyboard.</p>
</pct-dialog>
```
...
| **Inputs**      | `open` (`model`), `inline`, `heading`, `ariaLabel`, `ariaLabelledby`, `closeOnEscape`, `closeOnBackdrop`, `closeButton`                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| **Inline**      | `inline` draws the same panel where the tag stands instead of in an overlay — the same `<ng-template>`, so the same box, parts and stylesheet. It gives up everything that is the layer's: the `backdrop`, `aria-modal`, the focus trap (so `[pctAutofocus]` decides nothing and focus stays where the user left it), the scroll lock, `inert` on the background, and the place in the closing stack — which is the road Escape travels, so `closeOnEscape` and `closeOnBackdrop` govern nothing there. It keeps `role="dialog"`, the heading and both names, the close button, `closed` and `open` |
| **Parts**       | `backdrop`, `panel`, `header`, `heading`, `close`, `content` — an inline dialog draws every one of them but the `backdrop`, which is the layer's                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| **Harness**     | `PctDialogHarness`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| **SSR**         | nothing renders on the server — unless `inline`, which is what that input is for. The overlay panel is a template attached by a **browser** render, so a modal left `open` at bootstrap sends no markup and hydrates no mismatch; an inline panel is a branch of the template and arrives in the HTML the server sends                                                                                                                                                                                                                                                                              |
...
| part       | what it is                                       |
| ---------- | ------------------------------------------------ |
| `backdrop` | the scrim over the page — an overlay dialog only |
...
**The whole table is the overlay's.** An inline dialog is in no stack, so Escape passes it by and
`closeOnEscape` governs nothing; Tab walks in and straight out again, because the panel is text in
the page and there is no trap to hold it. Its ways out are the close button and whatever the
content holds (`libs/components/dialog/src/dialog.spec.ts`).
...
| Visual screenshot               | `apps/sandbox-e2e/src/visual.spec.ts` — `dialog-open` and `dialog-open-rtl`, the whole viewport rather than the panel: the veil over the page is part of what this component draws — with the sandbox's navigation blanked by the stage, so a row in that list is not a pixel here ([`req-quality-e2e`](../requirements/quality.md#req-quality-e2e))                                                                                                                                                                                                                                                           |
...
| SSR + hydration                 | `apps/sandbox-e2e/src/hydration.spec.ts` — the `/dialog` view renders and hydrates with no `NG05xx`; a closed dialog contributes no markup at all. The inline half is read in `libs/components/dialog/src/dialog.spec.ts` instead, under the flag Angular's own `afterNextRender` reads (`ngServerMode`): the inline panel is in the markup, the overlay panel is not                                                                                                                                                                                                                                          |
| Inline (`inline`)               | `libs/components/dialog/src/dialog.spec.ts` — twelve cases, and most of them read an ABSENCE: no veil, no `aria-modal`, no `inert`, no scroll lock, no capture, no Escape, and no warning about ways out that do not exist. Plus the crossing, which is the shape the input is for: a dialog that turns from a layer into a section while it is open keeps its state, hands the page back and emits no `closed` — it did not close, it changed where it draws                                                                                                                                                  |
...
| Focus: trap, capture, restore   | `apps/sandbox-e2e/src/dialog.spec.ts` — Tab never leaves the panel, focus goes back to the opener, and `pctAutofocus` decides what takes it. Measured in a browser and nowhere else: jsdom has no layout for the interactivity checker to read. Their ABSENCE inline needs no layout and is read in the unit spec: the keyboard does not move when the panel appears                                                                                                                                                                                                                                           |
| The background goes `inert`     | `apps/sandbox-e2e/src/dialog.spec.ts` (the pointer, the keyboard and a script all refused) + `libs/components/core/src/core.spec.ts` (**which** elements are marked). Two places for one behaviour, the same split `PctFocusStays` lives with. Inline nothing is marked at all, and the counter behind it stays at zero                                                                                                                                                                                                                                                                                        |
| The scroll lock                 | `apps/sandbox-e2e/src/dialog.spec.ts` — the wheel moves nothing while it is up and moves again after, plus the scrollbar's width handed back to the layout (visible in webkit alone; blink and gecko run here with overlay scrollbars and a gutter of 0) — the one coin toss this case ever showed (WebKit, 2026-09-02) was the READING of the baseline and not the lock, and the item that recorded it named the wrong step ([`lesson-149`](../lessons.md#lesson-149), [`lesson-164`](../lessons.md#lesson-164))                                                                                              |
...
| Screen-reader log               | Read 2026-09-16 from `docs/acr/at/` — three readers, whole sandbox, and **the answer is that the walk does not ask this question**. All three reach `/dialog` and read the triggers only — `Open the dialog · button.` (Orca), `Delete the project, button` (NVDA), `Open a form dialog button` (VoiceOver) — because the walk presses Tab and nothing else, so no modal in these logs was ever opened. What a reader announces when a modal opens, and whether a live region behind it still reaches the user, needs a pass that ACTS rather than walks. The gap is unchanged in substance and exact in shape |
| docs page                       | `/components/dialog` on the published site — prerendered, the demo's own source is the code tab                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |

## Decisions

[0066](../decisions/0066-a-panel-is-a-layer-or-a-region-and-the-consumer-says-which.md) (why `inline` exists, and why an inline dialog is not modal and answers no Escape),
...
mechanism),
[0047](../decisions/0047-a-drawer-is-a-region-of-the-page-not-a-layer-over-it.md) ("a region of
the page, not a layer over it" — the cut `inline` lets a consumer make per usage rather than
once per component), [0007](../decisions/0007-config-and-texts.md),
...
- **No stacking policy.** Two dialogs open at once work — the top layer orders them and the
  ...
  component does not.
- **Inline drops two of the three ways out, and nothing says so at runtime.** `closeOnEscape` and
  `closeOnBackdrop` govern nothing there — the first travels the closing stack an inline panel is
  not in, the second names a surface it does not draw — and no dev-mode message reports it: the
  responsive shape the input exists for BINDS `inline` and leaves both inputs at their defaults, so
  a warning would fire on half of every render and mean nothing on either half. The JSDoc and the
  row above say it instead.
- **An inline dialog announces itself to nobody.** No trap means no capture, so `[pctAutofocus]`
  decides nothing and the keyboard stays where the user left it. The panel keeps `tabindex="-1"`, so
  an application that wants to move focus to a section it has just revealed can — this component
  will not do it for them, because a region of a page that took the keyboard as it rendered would be
  the defect and not the feature.
- **No inline demo in the sandbox yet.** The browser battery above — axe, the two screenshots,
  forced colours, RTL, the scroll lock, the inert background — walks the sandbox, and there is no
  inline dialog in it, so every one of those rows is about the modal. What covers the mode today is
  the unit spec, and jsdom is enough for it: what inline does is mostly not doing things, and an
  absence needs no layout to be read.
