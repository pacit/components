...
| Screen-reader log                                          | Read 2026-09-16 from `docs/acr/at/` — three readers, whole sandbox. As for the container and the grid: **`/layout` produced no stop of its own in any of the three logs**. A stack is a gap between children and a reader walks the children, which it did — every stop on that view is one of the scaffold controls, announced by its own group |
| A docs page with live examples                             | `apps/sandbox/src/app/views/layout/` (the sandbox view). The published site: `/components/stack` — prerendered, the demo's own source is the code tab                                                                                                                                                                                            |
| Unit + mutation                                            | `libs/components/stack/src/stack.spec.ts` — 4 cases; targeted Stryker run 2026-09-02: **1 mutant, 1 killed — 100.00** (the `'md'` default swapped away dies on the default-gap case), and the day's shared full run wrote the same row into the snapshot: `stack.ts 100.00 1(0) 0 0 0`                                                           |
