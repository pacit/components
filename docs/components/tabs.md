...
| **Harness**     | `PctTabsHarness`, `PctTabHarness`                                                                                                                                                                                                                                                                   |
...
| axe audit                       | `apps/sandbox-e2e/src/a11y.spec.ts` — the `/tabs` view, LTR and RTL, with all four strips of the sandbox on the page at once (horizontal, manual, vertical, overflowing); and a stage of its own in the derived list of panels — a strip's panel is a `panel` part the sandbox holds open, so the stage holds the claim that one stands open in the walk rather than an audit of its own ([`lesson-196`](../lessons.md#lesson-196))                                                                                                                                                          |
...
| A tab stands in a tablist       | `tools/check-aria.mjs` point 9 — every `role="tab"` read off the template's tree with nothing but its `tablist` above it, axe's own `requiredContext` table being the rule ([`lesson-65`](../lessons.md#lesson-65))                                                                                                                                                                                                                                                                                                                                                                          |
...
| Screen-reader log               | Read 2026-09-16 from `docs/acr/at/` — three readers, whole sandbox. The tab and its panel are both announced, and the panel carries its content: Orca `General · page tab.` then `General · scroll pane clickable.`; NVDA `General, property page, The general settings, and a word that lives nowhere else on this page: marmalade.`; VoiceOver `General selected tab, 1 of 3 Account settings tab group`. **Find-in-page is not in these logs** — the walk presses Tab and nothing else — so what a reader says when a hidden panel is revealed and the tab under it changes is still open |
...
| docs page                       | `/components/tabs` on the published site — prerendered, the demo's own source is the code tab                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
