...
| Day arithmetic as laws          | `libs/components/date/src/day.property.spec.ts` — the step invertible and composing, the weekday anchored on the platform's own epoch, a month's length read from `Date`'s normalisation, an order agreeing with the millisecond difference, the grid six whole weeks containing its month, and the domain refused loudly at both ends. It found two defects no worked case had reached ([`lesson-185`](../lessons.md#lesson-185))                                                                                                                |
...
| Not a date, said                | `libs/components/date/src/date.spec.ts › junk in the field` — the sentence in the message line ahead of the form's, in the default wording and in the application's; `apps/sandbox-e2e/src/date.spec.ts › keeps junk in the field and reports it` — the same in three engines, in French, with `aria-describedby` naming it ([0070](../decisions/0070-what-the-control-knows-and-the-form-cannot-is-a-second-channel.md))                                                                                                                         |
| Screen-reader log               | Read 2026-09-16 from `docs/acr/at/` — three readers, whole sandbox. The INPUT is answered: `Start date · entry · 27/08/2026 · required. · Type it, or pick it from the calendar.` (Orca), `Day, edit, …, selected 27 slash 08 slash 2026` (NVDA), `27/08/2026 … Day … edit text` (VoiceOver), and the toggle says it opens a dialog in all three. **The grid is not**: the walk presses Tab and never opened the calendar, so whether every cell carries the whole date as its accessible name is the question left, and it is now exactly stated |
| docs page                       | `/components/date` on the published site — prerendered, the demo's own source is the code tab                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
And it says so: **`Not a date`** in the message line, ahead of whatever the form has to say,
because the form sees `null` and would call a required field empty while the user looks at
three numbers they typed. The sentence travels through the contract's second channel,
`ownErrors` ([0070](../decisions/0070-what-the-control-knows-and-the-form-cannot-is-a-second-channel.md)),
so a `pct-field` around the control shows it in the same place; it is the `dateMalformed`
text, so an application that translates the library is not told in English; and it is gated
by nothing — it exists only once the field was left, and a keystroke or a value from outside
takes it back.
...
- **The sentence comes first.** A control's own error stands ahead of the form's, always —
  there is no knob to let "required" win over "not a date" (0070).
