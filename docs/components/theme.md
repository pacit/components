...
| A docs page with live examples                             | `apps/sandbox/src/app/views/kitchen-sink/` (the `/all` view: the raw panel and the directive panel side by side). The published site: `/components/theme` — prerendered, the demo's own source is the code tab (2.1.7)              |
