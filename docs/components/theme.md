...
| Screen-reader log                                          | Read 2026-09-16 from `docs/acr/at/` — three readers, whole sandbox, and it is measured on every view rather than on one: the scaffold carries a theme switch to all thirty-six, and all three readers announce it as the radio group it is — `Theme · panel · light · selected radio button.` (Orca), `Theme, grouping` (NVDA), `light selected radio button, 1 of 2 Theme radio group` (VoiceOver). **The theme itself is never announced**: no reader said a word when the attribute changed the colours, which is the promise here |
| A docs page with live examples                             | `apps/sandbox/src/app/views/kitchen-sink/` (the `/all` view: the raw panel and the directive panel side by side). The published site: `/components/theme` — prerendered, the demo's own source is the code tab                                                                                                                                                                                                                                                                                                                        |
| Unit + mutation                                            | `libs/components/theme/src/theme.spec.ts` — 3 cases; in `mutation.policy.json` from day one — the day's shared full run generated zero mutants for one host binding, so the snapshot owes it no row until the directive grows logic                                                                                                                                                                                                                                                                                                   |
