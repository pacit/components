...
| Screen-reader log                                          | Read 2026-09-16 from `docs/acr/at/` — three readers, whole sandbox, **and it is the reading that changed the component**. The template argued the cross needed `texts().chipRemove` alone, the label beside it being heard anyway; all three said `Remove` at every one of five chips and named none of them ([`lesson-218`](../lessons.md#lesson-218)). The name is composed now, and re-read on the same three: `Remove In stock · button.` (Orca), `Remove Under 50, button` (NVDA), `Remove Free shipping button` (VoiceOver) — ten crosses on this view, each naming its own value. The named list and the item count arrive in all three. The FIRST cross is silent for NVDA and VoiceOver, and that is the instrument rather than the component: `enter` puts focus there outright, and neither reader announces a focus it did not see arrive |
