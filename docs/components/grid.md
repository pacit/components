...
| Screen-reader log                                          | Read 2026-09-16 from `docs/acr/at/` — three readers, whole sandbox. The same measurement as the container beside it: **`/layout` gave no stop of its own to any reader**, only the scaffold radio groups, so nothing a grid draws is a stop of the Tab key. A grid here is a column count in CSS and the logs agree by holding nothing                   |
| A docs page with live examples                             | `apps/sandbox/src/app/views/layout/` (the sandbox view). The published site: `/components/grid` — prerendered, the demo's own source is the code tab                                                                                                                                                                                                     |
