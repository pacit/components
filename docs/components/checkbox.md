...
| Screen-reader log               | Read 2026-09-16 from `docs/acr/at/` — three readers, whole sandbox. **`mixed` is announced by both readers that reach it, and in different words**: NVDA says `check box, half checked`, VoiceOver says `mixed checkbox`. Orca never reaches it — its walk of this view ends after one stop, focus leaving `main` for the scaffold — and its one reading is the required box: `check box not checked required · invalid entry · Required to open an account`. The READ-ONLY box is announced `checked` by NVDA and VoiceOver alike and neither says read-only, which is a finding rather than a limit of the instrument |
