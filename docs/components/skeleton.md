...
| `prefers-reduced-motion` — duration from a token           | `apps/sandbox-e2e/src/preferences.spec.ts` — the sheen reads **4500 ms** — three loops of the reduced token, the sheet's factor scaling the axis's answer rather than replacing it (the loop token slows, it does not stop: a still placeholder stops saying anything is coming). No media query stands in the component's sheet                                                             |
