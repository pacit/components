...
| Screen-reader log                                          | Read 2026-09-16 from `docs/acr/at/` — three readers, whole sandbox, and **the steps are not focusable, so a Tab walk does not reach them**. The logs hold the two buttons only: `Back · button.` (Orca), `Next, button` (NVDA). VoiceOver answered the same stop with its own instruction — `You are currently on a button. To click this button, press Control-Option-Space.` — which is the reader talking about itself and not about this component. What a reader makes of the step list needs a pass that reads rather than walks |
