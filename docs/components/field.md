...
| Parts in the inventory          | `libs/components/czesci.snapshot.md`, `tools/check-parts.mjs` (target `check-parts`). **11 parts while the requirement listed 7** until 2026-07-27; **15 while the card listed 11** until 2026-08-05 — four slot parts sit in the `host` blocks of directives. The same mistake twice on the same card, both times noticed only on counting |
