...
| **Harness**     | `PctFieldHarness`, `PctPrefixHarness`, `PctSuffixHarness`, `PctLabelAuxHarness`, `PctMessageAuxHarness`                                                                                                                                                                                                 |
...
| RTL                             | `apps/sandbox-e2e/src/rtl.spec.ts` — the affix measured on the other side of the control as GEOMETRY in both directions, which that file names its own reference control; plus the `field-affixes-rtl` and `field-aux-slots-rtl` baselines in `apps/sandbox-e2e/src/visual.spec.ts`, and `tools/check-styles.mjs` (point 5, [`req-token-logical`](../requirements/tokens.md#req-token-logical)) over the stylesheet                                                                                                                                                                             |
...
| Screen-reader log               | Read 2026-09-16 from `docs/acr/at/` — three readers, whole sandbox. The whole routed set arrives in one announcement: Orca gives `E-mail · entry · john@example.com · required. · invalid entry. · A work address.` — label, role, value, required, invalid, hint — and NVDA gives `A short description, edit, has auto complete, Briefly — write at least 10 characters, A few words about yourself, blank`, VoiceOver `A short description Briefly — write at least 10 characters edit text`. The label auxiliary is its own stop and announces as a button in all three, which is what it is |
| docs page                       | `/components/field` on the published site — prerendered, the demo's own source is the code tab                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
