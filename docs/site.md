...
- **Anything the site needs and the library lacks becomes library work first**, at the full
  definition of done (decision, tokens, unit, e2e in three engines, mutation, card).
...
| `/404`            | the page behind every address the site does not have — prerendered once, served by the host for any unknown path with a 404 status | hand-written page (site-local), decision 0078                     |
...
2. **The evidence strip — accessibility-led** (recalibrated 2026-09-02, on the
   ...
   **built and machine-audited to WCAG 2.2 AA** (never "conformant" — our gates end
   ...
   the axis worn on the front page, and the pipeline already emits both files.
...
of code moved: the second sketch's character with the first one's tables.
...
- **Evidence**: the component's own mutation score, e2e cases, colour pairs, baselines and
  what its preview costs to render (elements, listeners, renders, and a dated clock,
  [`req-quality-benchmark`](requirements/quality.md#req-quality-benchmark)) from the tracked
  snapshots; the card's design note — why it is built this way, with every
...
The same pass emits **`llms.txt` and the machine-readable catalogue**
([`req-api-catalogue`](requirements/api.md#req-api-catalogue)), which was always ordered
"built with the site, not after it". Being the first component library
