...
The brand gradient as equipment, on the element you already have: `edge` paints a rim, `text`
cuts the sweep out of the word, `fill` takes the surface. One face per element — the
combination nobody can read is not documented against, it is unrepresentable — and `show`
says whether it paints always or only under hover and `:focus-visible`.

```html
<article pctHero="edge" show="interact">…</article>
<h3 pctHero="text">Components that prove themselves</h3>
```
